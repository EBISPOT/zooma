...
/**
 * This repository takes information about the {@link Annotation}s that where queried and pivoted
...
public class CreateAnnotationSummariesRepository {
    ...
    /**
     * Converts {@link Annotation}s into {@link AnnotationSummary}s and calculated the summary scores using the pivot information.
     * Each {@link AnnotationSummary} will be represented by the strongest {@link Annotation} Document.
     *
     ...
     */
    public List<AnnotationSummary> convertToAnnotationSumaries(List<Annotation> content, List<FacetPivotFieldEntry> pivots, long totalDocumentsFound){
        ...
        for (FacetPivotFieldEntry propertyValues : pivots){
            ...
            for (FacetPivotFieldEntry semanticTag : semanticTags){
                ...
                ScoredPage<Annotation> docs = solrTemplate.queryForPage(idQuery, Annotation.class);
                List<Annotation> annotationsFromIds = docs.getContent();
                if (annotationsFromIds == null || annotationsFromIds.isEmpty()){
                    continue;
                }
                Annotation winnerAnnotation = annotationsFromIds.get(0); // highest quality
                ...
                AnnotationSummary annotationSummary = new AnnotationSummary(winnerAnnotation.getPropertyType(),
                        winnerAnnotation.getPropertyValue(),
                        winnerAnnotation.getSemanticTag(),
