...
/**
 * An Annotation DAO that is capable of reading assay annotations from the Gene Expression Atlas database.
 *
 * @author Tony Burdett
 * @author Simon Jupp
 * @date 03/04/2012
 */
public class AtlasAssayAnnotationDAO implements AnnotationDAO {
    ...
    public static final String ORDERING =
            "order by e.ACCESSION, a.ACCESSION, p.NAME, pv.NAME asc";
    ...
    public static final String ANNOTATIONS_SELECT_BY_BIOLOGICAL_ENTITY =
            ANNOTATIONS_SELECT + "where e.ACCESSION = ? and a.ACCESSION = ? " + ORDERING;
    ...
    public AtlasAssayAnnotationDAO() {
        this(new AtlasAnnotationFactory(new AtlasSampleLoadingSession()));
    }

    public AtlasAssayAnnotationDAO(AtlasAnnotationFactory annotationFactory) {
        this(new AtlasAnnotationMapper(annotationFactory));
    }

    public AtlasAssayAnnotationDAO(AtlasAnnotationMapper annotationMapper) {
