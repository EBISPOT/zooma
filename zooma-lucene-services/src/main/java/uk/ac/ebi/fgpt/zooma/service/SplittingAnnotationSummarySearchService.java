...
public class SplittingAnnotationSummarySearchService extends AnnotationSummarySearchServiceDecorator {
    ...
    public Map<AnnotationSummary, Float> searchAndScore(String propertyValuePattern) {
        ...
        Map<AnnotationSummary, Float> completeString_results = super.searchAndScore(propertyValuePattern);
        ...
        if(completeString_results.isEmpty()){
            ...
            if (getSearchStringProcessor_Splitter().canProcess(propertyValuePattern, null) ){
                ...
                List<String> partsString = getSearchStringProcessor_Splitter().processSearchString(propertyValuePattern);
                ...
                if(partsString!=null && partsString.size()==2){
                    ...
                    for(String partString:partsString){
                           ...
                           Map<AnnotationSummary, Float> partString_results = super.searchAndScore(partString);
                    ...
                    if( partResults.size()==2 ){
                        ...
                        Iterator it1 = partResults.get( partsString.get(0) ).entrySet().iterator();
                        ...
                        while(it1.hasNext()) {
                            ...
                            while(it2.hasNext()) {
                                ...
                                float scoreFinal = ((size_string1*score1 + size_string2*score2) / (size_string1+size_string2))*boost_partial_strings;
                                float qualityScoreFinal = ((size_string1*anSummary1.getQualityScore() + size_string2*anSummary2.getQualityScore() ) / (size_string1+size_string2)) *boost_partial_strings;
                                ...
                                if(anSummary1_semanticTags!=null && !anSummary1_semanticTags.isEmpty()){
                                    ...
                                    for (Iterator iter = anSummary1_semanticTags.iterator(); iter.hasNext(); ) 
                                        aggregation_semanticTags.add((URI)iter.next());

                                }

                                if(anSummary2_semanticTags!=null && !anSummary2_semanticTags.isEmpty()){

                                    for (Iterator iter = anSummary2_semanticTags.iterator(); iter.hasNext(); ) 
                                        aggregation_semanticTags.add((URI)iter.next());
                                ...
                                if(anSummary1_annotationURIs!=null && !anSummary1_annotationURIs.isEmpty()){
                                    ...
                                    for (Iterator iter = anSummary1_annotationURIs.iterator(); iter.hasNext(); ) 
                                        aggregation_annotationURIs.add((URI)iter.next());
                                ...
                                if(anSummary2_annotationURIs!=null && !anSummary2_annotationURIs.isEmpty()){
                                    ...
                                    for (Iterator iter = anSummary2_annotationURIs.iterator(); iter.hasNext(); ) 
                                        aggregation_annotationURIs.add((URI)iter.next());
    ...
    public Map<AnnotationSummary, Float> searchAndScore_QueryExpansion(String propertyValuePattern) {
        ...
        getLog().debug("SplittingAnnotationSummarySearchService: " + propertyValuePattern);
        getLog().debug("Calling searchAndScore... ");
        Map<AnnotationSummary, Float> completeString_results = super.searchAndScore_QueryExpansion(propertyValuePattern);
            ...
            if (getSearchStringProcessor_Splitter().canProcess(propertyValuePattern, null) ){
                ...
                if(partsString!=null && partsString.size()==2){
                    ...
                    for(String partString:partsString){
                           ...
                           Map<AnnotationSummary, Float> partString_results = super.searchAndScore_QueryExpansion(partString);
                           partResults.put(partString, partString_results);
                    }
   ...
   /* @Override 
    public Map<AnnotationSummary, Float> searchAndScore(String propertyType, String propertyValuePattern) {
        ...
        getLog().debug("SplittingAnnotationSummarySearchService: " + propertyValuePattern);
        getLog().debug("Calling searchAndScore... ");
        Map<AnnotationSummary, Float> completeString_results = super.searchAndScore(propertyType,propertyValuePattern);
            ...
            if (getSearchStringProcessor_Splitter().canProcess(propertyValuePattern, null) ){
                ...
                if(partsString!=null && partsString.size()==2){
                    ...
                    for(String partString:partsString){
                           ...
                           Map<AnnotationSummary, Float> partString_results = super.searchAndScore(propertyType,partString);
                           partResults.put(partString, partString_results);
                    }
    ...
    public Map<AnnotationSummary, Float> searchAndScore_QueryExpansion(String propertyType, String propertyValuePattern) {
        ...
        getLog().debug("SplittingAnnotationSummarySearchService: " + propertyValuePattern);
        getLog().debug("Calling searchAndScore... ");
        Map<AnnotationSummary, Float> completeString_results = super.searchAndScore_QueryExpansion(propertyType,propertyValuePattern);
            ...
            if (getSearchStringProcessor_Splitter().canProcess(propertyValuePattern, null) ){
                ...
                if(partsString!=null && partsString.size()==2){
                    ...
                    for(String partString:partsString){
                           ...
                           Map<AnnotationSummary, Float> partString_results = super.searchAndScore_QueryExpansion(propertyType,partString);
                           partResults.put(partString, partString_results);
                    }
